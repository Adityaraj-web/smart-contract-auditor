...
> **Update:** this project now includes a **Forensics extension** — post-incident, on-chain analysis of already-executed transactions, as a second mode alongside the original pre-deployment auditor. Summary below; full write-up (pipeline, bugs found, setup) is in [`docs/ENGINEERING_NOTES.md`](./docs/ENGINEERING_NOTES.md).
...
### Forensics Explorer
![Forensics report with decode summary, quality flags, and category assessments](./screenshots/05-forensics-landing.png)
...
The forensics extension adds a second, independent pipeline (transaction hash in, post-mortem report out) with its own registry contract — see [`docs/ENGINEERING_NOTES.md`](./docs/ENGINEERING_NOTES.md) for its architecture diagram.
...
*Forensics extension adds Etherscan API (transaction data) and `lossless-json` (uint256-safe parsing) — details in [`docs/ENGINEERING_NOTES.md`](./docs/ENGINEERING_NOTES.md).*
...
*Forensics-specific decisions (bitmask categories, attest-what-was-reviewed, dual-invocation-safe imports) are in [`docs/ENGINEERING_NOTES.md`](./docs/ENGINEERING_NOTES.md).*
...
│   ├── raw/, processed/              # SWC entries, exploits, concepts (audit)
...
│   ├── etherscan_client.py, forensics_ingest.py, log_decoder.py    # forensics
│   ├── timeline_builder.py, query_builder.py, attack_pattern_scoring.py
│   ├── forensics_prompt_builder.py, forensics_report_generator.py
│   └── blockchain_forensics_reconcile.py
├── explorer/                         # Next.js app
│   ├── app/
│   │   ├── page.tsx                       # audit UI
│   │   ├── attestations/page.tsx          # audit attestation explorer
│   │   ├── forensics/page.tsx             # forensics landing page
...
├── docs/
│   └── ENGINEERING_NOTES.md          # forensics deep-dive: pipeline, bugs, setup
└── README.md
```
...
**Want the forensics extension too?** (Etherscan key, extra env vars, forensics contract deploy, Supabase table, `lossless-json`) — setup steps are in [`docs/ENGINEERING_NOTES.md`](./docs/ENGINEERING_NOTES.md).
...
*Forensics walkthrough (paste a tx hash → review the report → attest) is in [`docs/ENGINEERING_NOTES.md`](./docs/ENGINEERING_NOTES.md).*
...
- Forensics extension has its own set of limitations (report caching, on-chain flag granularity, Etherscan rate limits) — see [`docs/ENGINEERING_NOTES.md`](./docs/ENGINEERING_NOTES.md).

---

## Author

Built solo by Adityaraj as a portfolio project exploring smart contract security, retrieval-augmented generation, and full-stack Web3 development — entirely on free-tier infrastructure.